...
export const MessageItem = memo(({item, setOpenImageView, setMessageMenuActive})=>{
    ...
    return (
        <Pressable style={{marginVertical:5, width:'100%'}} onPress={()=>setMessageMenuActive(false)}>
            <View style={{flexDirection:'row'}}><Image style={chat.profilepic} source={{uri:`${server}/api/getpfp?userid=${item.sender}`}}></Image><Text style={chat.msgusername}>{item.username}</Text></View>
                ...
                <View style={{flexDirection:'row', flexWrap:'wrap'}}>
                    {
                        ...
                        <Pressable key={index} style={{margin:5}} onPress={()=>{setOpenImageView([item.img, listitem.index])}} onLongPress={(PressEvent)=>setMessageMenuActive((oldvalue)=>!oldvalue && {...item, 'py':PressEvent.nativeEvent.pageY})}>
